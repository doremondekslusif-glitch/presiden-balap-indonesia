...
scene.fog = new THREE.Fog(0x68c8ed, 55, 180);
        ...
        .map(
          (k, i) =>
            `<li class="${k.isPlayer ? 'me' : ''}">
              <b>${i + 1}</b>
              <span>${k.character.avatar} ${k.character.name}</span>
              <em>${k.isPlayer ? 'ANDA' : 'AI'}</em>
            </li>`
        )
        .join('');
  }
